...
const HELP_SPECS = [
  ...
  },
  {
    ...
    notes: ['Use "zenifra help project <subcomando>" para detalhes de info, stop, resume, delete, url, logs, runs, cancelamento, metrics, capabilities, network, image, exposure, autoscaling, billing, envs e instances.'],
  },
  {
...
async function handleProjectInfo(session, flags) {
  ...
  if (flags.json) return printJson(projectInfoForOutput(
    isJobProject(project) ? sanitizePublicProject(project) : project,
  ));
